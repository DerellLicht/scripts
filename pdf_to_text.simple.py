...
Usage:
    python pdf_to_text_simple.py my_scanned_pdf_book.pdf
    python pdf_to_text_simple.py my_scanned_pdf_book.pdf -o output_name.txt
    python pdf_to_text_simple.py my_scanned_pdf_book.pdf --dpi 300 --lang eng
...
def main() -> int:
    ...
    # Get the page count up front via pdfinfo (fast metadata read, not a full
    # rasterization) so we can show progress immediately, then rasterize and
    # OCR one page at a time in the loop below -- instead of rasterizing the
    # entire book upfront, which meant a long silent wait before any output
    # appeared on longer books.
    ...
    for i in range(1, total + 1):
        ...
        try:
            ...
            print(f"\nError reading PDF: {e}", file=sys.stderr)
            return 1
